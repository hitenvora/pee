                        ...
                        <div class="breadcome-list">
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 text-end">
                                    ...
                                    <a class="btn btn-primary ms-3 add-company-master">
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body table-responsive p-0">
                                <table id="company-table-id" class="company-table">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>Name</th>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="modal-title" id="companytabs">Add Company Master</h2>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form class="row" method="post" id="companyform" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="company_master_id" id="companymasterid" value="">
                            <div class="col-lg-12">
...
@section('scripts')
    <script>
        $('.edit-form').hide();
        // add user form show
        $(document).on('click', '.add-company-master', function() {
            $('.modal-title').text('Add Company Master');
            $('#companymasterid').val('');
            $("#companyform")[0].reset();
            $('span[id$="_error"]').text('');
            $('.edit-form').hide();
        ...
        var dataTable = $('.company-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.get_company_master') }}",
                data: function(d) {
                    d._token = token;
                },
                type: 'POST',
        ...
        $('#companyform').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.insert_company_master') }}",
                headers: {
                ...
                success: (data) => {
                    if (data.status == 200) {
                        ...
                        if ($('#companymasterid').val() == '') {
                            toastr.success("Company master added successfully.");
                        } else {
                            toastr.success("Company master updated successfully.");
                        }
                        dataTable.draw();
                    } else {
        ...
        function editCompanyMaster(id) {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('admin/company-master-edit/') }}/" + id,
                headers: {
                    'X-CSRF-Token': token,
                },
                dataType: "json",
                success: (data) => {
                    $('.modal-title').text('Edit Company Master');
                    $("#companyform")[0].reset();
                    $('.edit-form').show();
                    // set edit value
                    $('#companymasterid').val(data.data.id);
                    $('#name').val(data.data.company_name);
                    // Show edit modal
                    $('#ajaxModel').modal('show');
                },
                error: function(response) {
        ...
        function daletetabledata(id) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('admin.delete_company_master') }}",
                        headers: {
                            'X-CSRF-Token': token,
                        },
                        data: _data,
