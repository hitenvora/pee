                            ...
                            <div class="breadcome-list">
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 text-end">
                                        ...
                                        <a class="btn btn-primary ms-3 add-product-master">
                ...
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h2 class="modal-title" id="suppliertabs">Add Product</h2>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form class="row" method="post" enctype="multipart/form-data" id="productform">
                                @csrf
                                <input type="hidden" name="product_master_id" id="productmasterid" value="">
                                <div class="col-lg-6">
                                ...
                                <div class="col-lg-6">
                                    <label class="form-label">Product Wieght</label>
...
@section('scripts')
    <script>
        $('.edit-form').hide();
        $(document).on('click', '.add-product-master', function() {
            $('.modal-title').text('Add Product Master');
            $('#productmasterid').val('');
            $("#productform")[0].reset();
            $('span[id$="_error"]').text('');
            $('.edit-form').hide();
        ...
        var dataTable = $('.product-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.get_product_master') }}",
                data: function(d) {
                    d._token = token;
                },
                type: 'POST',
        ...
        $('#productform').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.insert_product_master') }}",
                headers: {
                ...
                success: (data) => {
                    if (data.status == 200) {
                        ...
                        if ($('#productmasterid').val() == '') {
                            toastr.success("Product master added successfully.");
                        } else {
                            toastr.success("Product master updated successfully.");
                        }
                        dataTable.draw();
                    } else {
        ...
        function editProductMaster(id) {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('admin/product-master-edit/') }}/" + id,
                headers: {
                    'X-CSRF-Token': token,
                },
                dataType: "json",
                success: (data) => {
                    $('.modal-title').text('Edit Product Master');
                    $("#productform")[0].reset();
                    $('.edit-form').show();
                    // set edit value
                    $('#productmasterid').val(data.data.id);
                    $('#name').val(data.data.name);
                    $('#account_master_id').val(data.data.account_master_id);
                    $('#product_weightz').val(data.data.product_weight);
        ...
        function daletetabledata(id) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('admin.delete_product_master') }}",
                        headers: {
                            'X-CSRF-Token': token,
                        },
                        data: _data,
