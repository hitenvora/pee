                        ...
                        <div class="breadcome-list">
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 text-end">
                                    ...
                                    <a class="btn btn-primary ms-3 add-group-master">
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="modal-title" id="companytabs">Add Group Master</h2>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form class="row" method="post" id="groupform" enctype="multipart/form-data">
                            @csrf
...
@section('scripts')
    <script>
        ...
        $(document).on('click', '.add-group-master', function() {
            $('.modal-title').text('Add Group Master');
            $('#groupid').val('');
            $("#groupform")[0].reset();
            $('span[id$="_error"]').text('');
            $('.edit-form').hide();
            ...
            $.ajax({
                ...
                success: (data) => {
                    if (data.status == 200) {
                        ...
                        if ($('#groupid').val() == '') {
                            toastr.success("Group master addedd successfully.");
                        } else {
        ...
        function editGroupMaster(id) {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('admin/group-edit/') }}/" + id,
                headers: {
                    'X-CSRF-Token': token,
                },
                dataType: "json",
                success: (data) => {
                    $('.modal-title').text('Edit Group Master');
                    $("#groupform")[0].reset();
                    $('.edit-form').show();
                    // set edit value
