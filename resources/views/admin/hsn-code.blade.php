                        ...
                        <div class="breadcome-list">
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 text-end">
                                    ...
                                    <a class="btn btn-primary ms-3 add-hsn-code">
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body table-responsive p-0">
                                <table id="hsn_code-table-id" class="hsncodetable">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>HSN Code</th>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="modal-title" id="companytabs">Add HSN Code</h2>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form class="row" method="post" id="hsnCodeForm" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="hsn_code_id" id="hsn_code_id" value="">
                            <div class="col-lg-12">
                                <label for="inputtitle1" class="form-label">HSN Code</label>
...
@section('scripts')
    <script>
        $('.edit-form').hide();
        $(document).on('click', '.add-hsn-code', function() {
            $('.modal-title').text('Add HSN Code');
            $('#hsn_code_id').val('');
            $("#hsnCodeForm")[0].reset();
            $('span[id$="_error"]').text('');
            $('.edit-form').hide();
        ...
        var dataTable = $('.hsncodetable').DataTable({
            processing: true,
            serverSide: true,
            ordering: true,
            autoWidth: false,
            ...
            $.ajax({
                ...
                success: (data) => {
                    if (data.status == 200) {
                        ...
                        if ($('#hsn_code_id').val() == '') {
                            toastr.success("HSN Code addedd successfully.");
                        } else {
                            toastr.success("HSN Code updated successfully.");
                        }
                        dataTable.draw();
                    } else {
        ...
        function editHSNCode(id) {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('admin/hsn-code-edit/') }}/" + id,
                headers: {
                    'X-CSRF-Token': token,
                },
                dataType: "json",
                success: (data) => {
                    $('.modal-title').text('Edit HSN Code');
                    $("#hsnCodeForm")[0].reset();
                    $('.edit-form').show();
                    // set edit value
                    $('#hsn_code_id').val(data.data.id);
                    $('#hsn_code').val(data.data.hsn_code);
        ...
        function daletetabledata(id) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('admin.delete_hsn-code') }}",
                        headers: {
                            'X-CSRF-Token': token,
                        },
                        data: _data,
