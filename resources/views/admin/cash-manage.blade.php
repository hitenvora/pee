                    ...
                    <div class="modal-body">
                        <form class="row" method="post" enctype="multipart/form-data" id="cashmanageform">
                            ...
                            <div class="col-lg-6">
                                ...
                                <span class="text-danger" id="create_date_error"></span>
                            </div>
...
@section('scripts')
    <script>
        ...
        var dataTable = $('.account-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.get_account-cash-entry') }}",
                data: function(d) {
                    d._token = token;
                },
                type: 'POST',
        ...
        $('#cashmanageform').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.insert_account-cash-entry') }}",
                headers: {
        ...
        function editAccountCashManage(id) {
            $('span[id$="_error"]').text('');
            $.ajax({
                type: 'GET',
                url: "{{ url('admin/account-cash-entry-edit/') }}/" + id,
                headers: {
                    'X-CSRF-Token': token,
                },
                dataType: "json",
                success: (data) => {
                    $('.modal-title').text('Edit Account Cash Entry');
                    $("#cashmanageform")[0].reset();
                    $('.edit-form').show();
                    // set edit value
                    $('#cash_manage_id').val(data.data.id);
                    ...
                    $('#create_date').val(data.data.date)
                    // Show edit modal
                    $('#ajaxModel').modal('show');
                },
                error: function(response) {
        ...
        function daletetabledata(id) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('admin.delete_account-cash-entry') }}",
                        headers: {
                            'X-CSRF-Token': token,
                        },
                        data: _data,
